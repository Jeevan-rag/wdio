...
When(/^I login with valid user credentials$/, function () {
    ...
    var user=$("//input[@type='password']").setValue("abc3940");
    //browser.pause(3000);
...
Then(/^I should be logged in successfully$/, function () {
   ...
   //browser.pause(5000);
   browser.$("//a[@id='global-user-trigger']").click();// container
   //clicks on logout 
   var logt=browser.$('//*[@id="global-header"]/div[2]/ul/li[2]/div/div[1]/ul[1]/li[5]/a').click();
   browser.pause(3000);
});
...
When(/^I login with invalid user credentials$/, function () {
    ...
    browser.pause(3000);
    browser.$('//*[@id="did-ui-view"]/div/section/section/form/section/div[3]/button').click();
    browser.pause(3000);
